...
export class PlayerDirector {
  ...
  position(id: string, role: Role, x: number, z: number, visualProfileId?: string, name?: string): void {
    const v4Role: PlayerRole =
      role === 'BATTER' ? 'BATTER' : role === 'BOWLER' ? 'BOWLER' : role === 'KEEPER' ? 'KEEPER' : 'FIELDER';
  ...
  state(id: string, _role: Role, state: PresentationState): void { this.v4Rig.transitionPlayer(id, state as any); }
  offset(id: string, offset: THREE.Vector3): void { this.v4Rig.setPlayerPresentationOffset(id, offset); }
  update(dt: number, _time?: number): void { this.v4Rig.update(dt); }
  reset(): void { this.v4Rig.resetForNextBall(); }
}
