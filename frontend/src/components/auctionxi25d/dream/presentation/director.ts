...
export class DreamMatchPresentation {
  ...
  get isDeliveryPreview(): boolean {
    return this.previewOnly;
  }

  /** Starts only the client-visible delivery flight. No cricket outcome is invented here. */
  startDeliveryPreview(e: AuthoritativeBallEvent) {
    this.current = e;
    this.elapsed = 0;
    this.previewOnly = true;
    this.timeline.reset();
    this.fielding.reset();
    this.ball.beginDeliveryPreview(e);

    if (e.bowlerId) this.players.state(e.bowlerId, 'BOWLER', 'RUNUP');
    if (e.strikerId) this.players.state(e.strikerId, 'BATTER', 'READ');

    const trajectory = this.ball.getTrajectory();
    if (trajectory) this.camera.set('DELIVERY_TRACK', true);
  }

  endDeliveryPreview() {
    this.previewOnly = false;
    this.ball.endPreview();
    this.camera.clearManualOverride();
  }
  ...
  playBall(e: AuthoritativeBallEvent) {
    ...
    if (resolvedTrajectory) {
      this.camera.beginBall(e, timings, resolvedTrajectory, this.fielding.getSequence(), this.fielding.getTarget());
    }
    ...
    this.timeline
      ...
      .add('release', Math.min(0.34, releaseTime), Math.min(0.62, releaseTime + 0.22), (t) => {
        if (e.bowlerId && t > 0.35) this.players.state(e.bowlerId, 'BOWLER', `RELEASE_${e.deliveryKind}` as any);
      })
  ...
  update(dt: number, cam: THREE.PerspectiveCamera) {
    this.elapsed += dt;
    if (!this.previewOnly) this.timeline.tick();
    this.ball.update(this.elapsed);
    if (!this.previewOnly) this.fielding.update(this.elapsed);
