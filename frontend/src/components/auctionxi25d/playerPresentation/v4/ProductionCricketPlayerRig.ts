...
import {AUCTION_XI_FRIEND_PROFILES} from './assetManifest';
...
function poseFor(state:PresentationState,t:number,a:PlayerArchetype):TransformPose{
  ...
  const power=a==='AGGRESSIVE_BATTER'?1.18:1,fast=a==='FAST_BOWLER'?1.2:1,s=Math.sin(t*Math.PI),c=Math.cos(t*Math.PI*2);
...
export class ProductionCricketPlayerRig{
  ...
  private hybrid:HybridCricketAssetAdapter|null=null;
  ...
  static resolveFriend(nameOrId?:string,jerseyNumber?:number){
    const q=(nameOrId||'').trim().toLowerCase();
    const p=AUCTION_XI_FRIEND_PROFILES.find(x=>x.likenessId.toLowerCase()===q||x.displayName.toLowerCase()===q||String(x.jerseyNumber)===String(jerseyNumber??''));
    return p?profileToIdentity(p,'AUCTION_XI'):null;
  }
  registerFriendProfiles(teamCode='AUCTION_XI'){return AUCTION_XI_FRIEND_PROFILES.map(p=>profileToIdentity(p,teamCode));}
  addFriendPlayer(likenessId:string,position:THREE.Vector3,yaw=0,teamCode='AUCTION_XI'){
  ...
  addPlayer(input:PlayerIdentity,position:THREE.Vector3,yaw=0){
    ...
    const matched=AUCTION_XI_FRIEND_PROFILES.find(p=>
      (input.visualProfileId && p.likenessId===input.visualProfileId) ||
    ...
    asset.root.userData.playerName=identity.name;asset.root.userData.jerseyNumber=identity.jerseyNumber;asset.root.userData.teamCode=identity.teamCode;asset.root.userData.likenessId=identity.likenessId;asset.root.userData.visualProfileId=identity.visualProfileId;
    ...
    if(this.hybrid&&identity.assetUrl){
      void this.hybrid.ensureLoaded(identity).then(real=>{
        const current=this.players.get(identity.id);if(!real||!current)return;
        real.root.position.copy(current.asset.root.position);real.root.rotation.copy(current.asset.root.rotation);real.setState(current.state);
        this.group.remove(current.asset.root);current.asset.dispose();this.group.add(real.root);current.asset=real;
      });
    }
  }
