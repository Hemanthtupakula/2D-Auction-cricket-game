...
export const stateDuration=(state:PresentationState)=>state.startsWith('RELEASE_')?0.36:['DRIVE','CUT','PULL','FLICK','SWEEP','LOFT','DEFENSIVE'].includes(state)?0.55:['DIVE','CATCH','THROW','PICKUP'].includes(state)?0.48:state==='CELEBRATE'?1.4:0.35;
